          ...
          <div className="rounded-3xl bg-white/70 border border-[#D4AF37]/30 p-6 sm:p-10">
            ...
            <div className="mt-8 grid md:grid-cols-3 gap-6">
              {(content?.deliverySteps ?? []).map((s: { title: string; desc: string }, idx: number) => (
                <div
                  key={s.title}
                  className="p-7 rounded-2xl bg-white/75 border border-[#D4AF37]/30"
                >
                  <div className="w-10 h-10 rounded-xl bg-[#BFA45F] flex items-center justify-center font-bold">
                    {idx + 1}
                  </div>
                  <h3 className="mt-5 font-bold text-[#0f172a]">{s.title}</h3>
                  <p className="text-[#67552b] mt-2 leading-relaxed">{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-white/70 border border-[#D4AF37]/30 p-10">
            ...
            <div className="mt-8 grid md:grid-cols-3 gap-6">
              {(content?.whyChoose ?? []).map((w: { title: string; desc: string }) => (
                <div
                  key={w.title}
                  className="group relative overflow-hidden rounded-2xl border border-[#D4AF37]/30 bg-white/75 p-7 transition-all duration-300 hover:-translate-y-2 hover:border-[#f6df9a]/50 hover:shadow-[0_0_35px_rgba(212,175,55,0.18)] active:-translate-y-1 active:border-[#f6df9a]/60"
                >
