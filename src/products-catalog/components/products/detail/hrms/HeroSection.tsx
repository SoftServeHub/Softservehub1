...
const product = topProducts.find((p) => p.id === "hrms")!;
...
const HeroSection = () => (
  <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
    ...
    <div className="absolute top-1/3 right-1/4 w-96 h-96 rounded-full bg-purple-500/8 blur-3xl pointer-events-none" />
    <div className="absolute bottom-1/4 left-1/4 w-72 h-72 rounded-full bg-pink-600/8 blur-3xl pointer-events-none" />
    ...
    <div className="relative z-10 max-w-5xl mx-auto px-6 w-full">
      <div className="flex flex-col items-center text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-600 mb-8 shadow-2xl shadow-purple-500/30"
        >
        ...
        >
          <span className="text-white">Intelligent HR</span>
          <br />
          <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">For Modern Teams</span>
        </motion.h1>
        ...
        >
          {product.techStack.map((t, i) => (
            <span key={i} className="px-3 py-1 rounded-full border border-purple-500/30 bg-purple-500/5 text-purple-300 text-xs font-medium">
              {t}
            </span>
          ))}
        </motion.div>
